...
# Get service URL
SERVICE_URL=$(kubectl get ksvc ml-predictor -o jsonpath='{.status.url}' 2>/dev/null || echo "http://localhost:8080")
...
for i in {1..10}; do
    ...
    kubectl run -it --rm traffic-gen-$i --image=curlimages/curl --restart=Never -- \
      ...
      -d "{\"features\": [$(shuf -i 1-10 -n 1).0, $(shuf -i 1-10 -n 1).0, $(shuf -i 1-10 -n 1).0]}" &
    sleep 1
done
